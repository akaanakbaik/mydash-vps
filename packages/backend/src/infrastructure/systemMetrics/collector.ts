...
export async function recordSystemMetricsSnapshot(
  ...
  const writes = [
    ...
    ingest.execute({ ...common, metricType: 'disk', data: { device: '/', filesystem: system.filesystem, mountPoint: '/', totalBytes: system.totalDiskMB * 1024 * 1024, usedBytes: system.usedDiskMB * 1024 * 1024, availableBytes: system.freeDiskMB * 1024 * 1024, usedPercent: system.diskUsagePercent, inodeTotal: 0, inodeUsed: 0, readSpeedBps: 0, writeSpeedBps: 0, ioWaitPercent: 0, smartStatus: null, topDirectories: system.diskTopDirectories, uptimeSeconds: system.uptimeSeconds, bootTime: system.bootTime } }, context),
    ingest.execute({ ...common, metricType: 'network', data: { interfaceName: system.network.interface, publicIpv4: '', publicIpv6: null, macAddress: '', rxBytesPerSec: system.network.rxSpeed * 1024 * 1024, txBytesPerSec: system.network.txSpeed * 1024 * 1024, packetLossPercent: 0, latencyMs: 0, connectionCount: 0, establishedCount: 0, rateAvailable: system.network.rateAvailable, rxBytes: system.network.rxBytes, txBytes: system.network.txBytes, interfaces: system.networkInterfaces, uptimeSeconds: system.uptimeSeconds, bootTime: system.bootTime } }, context),
