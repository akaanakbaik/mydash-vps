  ...
  async execute(input: { serverId: string; mode: string }, context: UseCaseContext): Promise<Result<unknown, AppError>> {
    try {
      const record = {
        ...
        mode: input.mode as import('@mydash/shared').BackupMode, // eslint-disable-line @typescript-eslint/consistent-type-imports
      ...
      await this.repository.saveRecord(record as import('../../domain/backup/index.js').BackupRecord); // eslint-disable-line @typescript-eslint/consistent-type-imports
