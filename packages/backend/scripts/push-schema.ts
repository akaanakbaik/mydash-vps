 ...
 * Run with: DATABASE_URL=postgresql://... npx tsx scripts/push-schema.ts
 */
import pg from 'pg';
...
async function main() {
  ...
  try {
    ...
    // Create the uuid-ossp extension if available (not critical if it fails)
    ...
    // Additional schema tables (monitoring, analytics, automation, etc.)
    const extraTables = [
