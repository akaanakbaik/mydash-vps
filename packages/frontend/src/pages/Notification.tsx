...
export function NotificationPage() {
  ...
  const notificationData = {
    ...data,
    summary: data.summary ?? { totalSent: 0, delivered: 0, failed: 0, pending: 0, successRate: 0, rateLimited: 0 },
    queue: data.queue ?? { pending: 0, processing: 0, throughput: 0, avgWaitTime: 0, maxQueueSize: 0, backpressure: false },
    retry: data.retry ?? { activeRetries: 0, maxRetries: 0, successRate: 0, avgRetryDelay: 0, nextRetryBatch: '', deadLetterCount: 0 },
    rateLimit: data.rateLimit ?? { currentRate: 0, limit: 0, remaining: 0, resetAt: '', throttled: 0 },
    deduplication: data.deduplication ?? { enabled: false, window: 0, deduplicated: 0, recentHashes: 0 },
  ...
  const filteredHistory = (() => {
    let rows = notificationData.history;
    if (searchQuery) {
  ...
  return (
    <PageContainer maxWidth="xl">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <NotificationProviderStatus providers={notificationData.providers} />
        </div>
      </div>
      <NotificationOverview summary={notificationData.summary} className="mb-6" />
      <DashboardGrid cols={1} colsMd={2} colsLg={3} gap="gap-4" className="mb-6">
        <NotificationSummaryCard summary={notificationData.summary} />
        <NotificationRuleCard rules={notificationData.rules} />
        <NotificationProviderCard providers={notificationData.providers} />
      </DashboardGrid>
      <DashboardSection title="Delivery System" subtitle="Queue, retry, rate limiting, and deduplication status" className="mb-6">
        <DashboardGrid cols={1} colsSm={2} colsLg={4} gap="gap-4">
          <NotificationQueueCard queue={notificationData.queue} />
          <NotificationRetryCard retry={notificationData.retry} />
          <NotificationRateLimitCard rateLimit={notificationData.rateLimit} />
          <NotificationDeduplicationCard dedup={notificationData.deduplication} />
        </DashboardGrid>
      </DashboardSection>
      <DashboardGrid cols={1} colsLg={2} gap="gap-4" className="mb-6">
        <NotificationDeliveryCard stats={notificationData.deliveryStats} />
        <NotificationTemplateCard templates={notificationData.templates} />
      </DashboardGrid>
      <DashboardGrid cols={1} colsLg={2} gap="gap-4" className="mb-6">
        <DashboardWidgetContainer title="Notification Timeline" subtitle="24-hour activity">
          <NotificationTimeline data={notificationData.timeline} label="Notifications Over Time" />
        </DashboardWidgetContainer>
        <DashboardWidgetContainer title="Recent Activity" subtitle="Latest notification events">
          <NotificationActivity activity={notificationData.activity} />
        </DashboardWidgetContainer>
      </DashboardGrid>
      <DashboardSection title="Delivery History" subtitle={`${String(filteredHistory.length)} events`}>
        <DashboardWidgetContainer title="History" subtitle={`${String(filteredHistory.length)} records`}>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <NotificationFilter options={notificationData.filterCategories} selected={categoryFilter} onChange={setCategoryFilter} />
