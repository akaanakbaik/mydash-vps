...
export class ConnectionManager {
  ...
  private dispatchToHandlers(event: RealtimeEvent): void {
    for (const handler of this.eventHandlers) {
      try { handler(event); } catch { /* one bad handler */ }
    }
  }

  private notifyStatusHandlers(): void {
    for (const handler of this.statusHandlers) {
      try { handler(this.status, this.previousStatus); } catch { /* one bad handler */ }
    }
  }
  ...
  private notifyStateHandlers(): void {
    ...
    for (const handler of this.stateHandlers) {
      try { handler(state); } catch { /* one bad handler */ }
    }
  }
