...
describe('EventBus', () => {
  ...
  describe('Memory Growth Pruning', () => {
    ...
    it('should prune 10% of oldest events when exceeding 10K threshold', () => {
      // Dispatch 10,100 events to go past the 10K threshold
      ...
      // After pruning: size should be ~10,100 - 1,010 = ~9,090 (with buffer for intermediate dispatches)
      ...
      // Oldest event IDs should have been removed (pruning removes ~1,000 oldest)
      expect(bus.hasProcessed('prune-mass-0')).toBe(false);
      expect(bus.hasProcessed('prune-mass-500')).toBe(false);

      // Events beyond the pruned range should still be present
